...
public final class FileHelper {
		...
		if (mkdirs) {
			...
			if ((parentDestFile != null) && (!parentDestFile.exists())) {
				if (!parentDestFile.mkdirs()) {
					throw new EventBridgeException("Could not create directory " + parentDestFile.getPath());
				}
			}
		}
	...
	public static void delete(File file) {
		if (!file.delete()) {
			throw new EventBridgeException("Could not delete file " + file.getPath());
		}
	}
	...
	public static void createDirectory(File file, boolean chain) {
		...
		EventBridgeException ex = new EventBridgeException("Could not create directory " + file.getPath());
		if (chain) {
			if (!file.mkdirs()) {
				throw ex;
			}

...
public class ConfigHelperTest {
	...
	public void testInputConfigValidationFile() {
		...
		try {
			ConfigHelper.inputConfigValidation(new InputConfig().withFile(new File("/non/existent/file")));
		} catch (DeferoException ex) {
			Assert.assertEquals("Input file '/non/existent/file' does not exist.", ex.getMessage());
		}
		
		try {
			ConfigHelper.inputConfigValidation(new InputConfig().withFile(new File("src/test/resources")));
		} catch (DeferoException ex) {
